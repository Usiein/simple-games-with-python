...
def getNewBoard():
    ...
    for x in range(60): # the main list is a list of 60 lists
        board.append([])
        for y in range(15): # each list in the main list has 15 single-char str
...
def makeMove(board, chests, x, y):
    ...
    # Return False if this is invalid move
...
def enterPlayerMove():
    # Let the player type in his move. Return a two-item list of int xy coords
...
(0 - 59)(0 - 14)(or type Quit)')
    while True:
        ...
        print('Enter a number from 0 to 59, space, then a number from 0 to 14.')
...
def showInstructions():
    print('''Instructions:
You are the captain of the Hispaniola, a treasure-hunting ship. Your current
mission is to find the three sunken treasure chests that are lurking in the
part of the ocean you are in and collect them.
...
chest up to a distance of 9. If all chests are out of range, the point will
be marked with O.
If a device is directly dropped on a treasure chest, you have discovered the
location of the chest, and it will be collected. The sonar device will remain
there.
...
print('S O N A R !')
print()
if input('Would you like to view the instructions? (Y/N)').lower().startswith('y'):
    showInstructions()
...
while True:
    ...
    while sonarDevices > 0:
        ...
        if moveResult == False:
            ...
            if moveResult == 'You have found sunken treasure chest!':
                # update all the sonar devices currently in the map
...
Congradulations and good game!')
            break

        sonarDevices -= 1

    if sonarDevices == 0:
        print('We\'ve run out of sonar devices! Now we had to turn ship \
around and head for home with treasure chests still out \
there! Game over.')
